...
Per prima cosa, è stata dedicata attenzione alla creazione del db. Infatti, come accadeva nel docker-compose, l’obbiettivo è quello di istanziare dei db, indipendenti dal be, con dei dati pre caricati.
Nel dettaglio ogni Database è composto da i seguenti volumi di k8s:
\begin{enumerate}
    \item ConfigMap che ....
    \item Due PersistentVolumeClaim che sono necessari per salvare il file init.db e salvare i dati del dapibus
    \item Deployment 
    \item Service
\end{enumerate}

Mentre ogni back-end è composto da un deployment e da un service. Per ogni microservizio vorremo instanziare 2 repliche, 3 per keycloak ed 1 solamente per il database (inserisci motivazione)

\section{AWS}

La parte finale, se non la principale, del progetto è stata dedicata per il deploy su aws.
È stato trovato opportuno utilizzare solamente il servizio di EKS [....inserisci cosa fa....]. Il deploy è stato gestito utilizzando \texttt{kubectl}, \texttt{aws cli}, \texttt{eksctl},
nel README.md saranno citate le guide ed i comandi utilizzati per la configurazione e set-up.

Per prima cosa è stato creato un cluster t2.xlarge con 4 nodi, in quanto il t2.micro risultava troppo piccolo.

Sono stati individuati dei namespace di riferimento per ogni microservizio di riferimento, ovvero: \textit{biobank}, \textit{donor}, \textit{sample}, \textit{shipment}, \textit{sprecsample}.  
È stato previsto, inoltre, un \textbf{Secret volume} necessario per settare le credenziali di accesso per ogni namespace.
